...
# ==========================================
@st.cache_data(ttl=3600)  # 1시간 캐싱하여 다수 접속 시 API 차단 방지 및 속도 향상
def get_realtime_data():
    # 티커 매핑 (밀: ZW=F, 옥수수: ZC=F, 콩: ZS=F, 쌀: ZR=F, WTI: CL=F, 브렌트유: BZ=F, 환율: USDKRW=X)
    tickers = {
    ...
    start_date = end_date - timedelta(days=365 * 3) # 3년치 시계열 확보
...
try:
    ...
    prev_year = df_raw.iloc[-252] if len(df_raw) > 252 else df_raw.iloc[0]
except:
    st.error("실시간 데이터를 불러오는 중 오류가 발생했습니다. 잠시 후 새로고침해 주세요.")
    st.stop()
...
with main_col_right:
    ...
    * <span class="news-tag">시장동향</span> 환율 상승 압력에 따른 주요 수입국별 선도구매 물량 헷징 비중 조절 움직임 포착
    ...
    macro_data = pd.DataFrame({
        ...
        '전일 대비\n증감': [f"{calc_chg(latest['WTI'], prev_day['WTI']):+.1f}%", f"{calc_chg(latest['브렌트'], prev_day['브렌트']):+.1f}%", "-1.0%", "+0.6%", f"{calc_chg(latest['환율'], prev_day['환율']):+.1f}%"],
        '전년 대비\n증감': [f"{calc_chg(latest['WTI'], prev_year['WTI']):+.1f}%", f"{calc_chg(latest['브렌트'], prev_year['브렌트']):+.1f}%", "+12.4%", "-3.1%", f"{calc_chg(latest['환율'], prev_year['환율']):+.1f}%"]
    })
    
    st.dataframe(macro_data, use_container_width=True, hide_index=True)

# ==========================================
# 5. 하단 수입 추이 영역 (식용 / 사료용 구분 테이블)
# ==========================================
